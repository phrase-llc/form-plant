...
`wrangler.toml` is **gitignored** — copy `wrangler.toml.sample` to `wrangler.toml` and fill it in before running `npm run dev`. Required `[vars]`: `AWS_REGION`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `SES_FROM_ADDRESS`, `SES_TO_ADDRESS`, `TURNSTILE_SECRET_KEY`, `ALLOWED_ORIGINS`. Optional: `SITE_LABEL`, appended to the mail subject.
...
Turnstile is the only gate on submissions, and nothing limits how many times it can be cleared. There is no rate limiting yet.
