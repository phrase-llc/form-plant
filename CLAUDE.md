...
**Whether a pattern can backtrack catastrophically is not checked here, and that is deliberate.** It is checked in the console, when the pattern is saved. The danger is real: a quantifier applied to a group that already contains one costs exponential time in the input length, and the regex runs on visitor-supplied input. Measured, `^(\d+)+$` against a 35-character value burned 72 seconds of CPU in the Function, and in the browser, where there is no CPU limit, it freezes the tab outright. Nor does it take a hostile tenant — that pattern matches a real phone number instantly and only blows up on long input that *fails* to match, so it behaves perfectly until a visitor mistypes.

The check still belongs at the write side. Only the console writes KV, so a pattern that reached KV has already been vetted, and this endpoint already trusts those same records for the SES sender address and the Turnstile secret — singling out the regex would be inconsistent. Rejecting at read time also tells the wrong person: the visitor gets a 500 and whoever wrote the pattern learns nothing, whereas in the console the error lands on the author at the moment they can fix it. Length limits are not a mitigation for any of this and must not be mistaken for one; `MAX_FIELD_LENGTH` bounds the mail body, nothing more.

What this leaves uncovered is hand-seeded KV: someone who writes records with `wrangler kv key put` instead of going through the console gets a submit endpoint that stalls, with nothing in this repo to warn them.
